...
package com.kaaterskil.utilities{
	...
	public class Alert_window extends Sprite{
		...
		private function draw_confirm_item(obj:Object):void{
			var form_type:String	= obj.type;
			var prams:Object		= obj.prams;
			
			switch(form_type){
				case 'confirm':
					var txt_obj:Text_field = new Text_field(prams);
					break;
				case 'text':
					var txt_obj:Text_input = new Text_input(prams);
					_fields.push(txt_obj);
					break;
				case 'radio_button':
					var radio_obj:Radio_button = new Radio_button(prams);
		...
		private function draw_other_btn():void{
			...
			_cancel_btn.label	= this.props.button_txt;
			_cancel_btn.name	= 'other_btn';
			_cancel_btn.width	= 100;
			_cancel_btn.addEventListener(MouseEvent.CLICK, other_handler);
		...
		private function draw_cancel_btn():void{
			var x_coord:Number = _ok_btn.x - 90;
			if(_container.contains(_other_btn)){
				x_coord = _other_btn - 110;
			}
			var y_coord:Number = _ok_btn.y;
