...
package com.kaaterskil.utilities{
	...
	public class Payment extends EventDispatcher{
		/**
		 * Trackers
		 */
		...
		------------------------------------------------------------*/
		/**
		 * Lists the available payment modules in comma-delimited format.
		 * This string will be converted to an array.
		 */
		...
		/**
		 * Sets the Authorize,net status.
		 */
		public static const PAYMENT_MODULE_AUTHORIZENET_STATUS = 'False';
		
		/**
		 * Sets the cash payment status.
		 */
		public static const PAYMENT_MODULE_CASHPMT_STATUS = 'Enabled';
		 ...
		 * Constructor
		 */
		public function Payment(module:String):void{
			_selected_module = '';
			
			if(PAYMENT_MODULES_INSTALLED != ''){
				//build array of installed modules
				var module_names:Array = PAYMENT_MODULES_INSTALLED.split(',');
				...
				for(i = 0; i < include_modules.length; i++){
					try{
						...
						var class_name:String = 'com.kaaterskil.utilities.' + include_modules[i];
				...
				terminate(Payment.ERROR_NO_PMT_MODULES_INSTALLED);
				return;
			}
		}
		 ...
		 * @param test		Test is only the selected module should be set
		 */
		...
		public function pre_confirmation(obj:Object):String{
			...
			if(_selected_module != ''){
				...
				r = Payment.ERROR_PMT_MODULE_NOT_FOUND;
			}
			return r;
		}
