...
package com.kaaterskil.utilities{
	...
	public class Combo_box extends Sprite{
		...
		private function init(obj:Object):void{
			...
			//set selected index
			var str:String = obj.selected_item != undefined ? obj.selected_item : '';
			set_selected_index(str);
		}
		...
		/**
		 * Sets the drop-down index to a match with a specified value
		 */
		private function set_selected_index(v:String):void{
			if(v != ''){
				for(var i:int = 0; i < _field.length; i++){
					_field.selectedIndex = i;
					if(_field.selectedItem.data != undefined && _field.selectedItem.data == v){
						break;
					}
					if(_field.selectedItem.value != undefined && _field.selectedItem.value == v){
						break;
					}
				}
			}else{
