...
package com.kaaterskil.payment{
	...
	public class Cash_payment extends Sprite{
		...
		private function init():void{
			...
			_amount				= 0;
			_module_title		= Constants.CASHPMT_FORM_LABEL;
			_module_description	= Constants.CASHPMT_FORM_DESCRIPTION;;
			_order_status		= Constants.CASHPMT_ORDER_STATUS;
			_payment_method		= Cash_payment.CASHPMT_TEXT_TITLE;
			_is_enabled			= Constants.PAYMENT_MODULE_CASHPMT_STATUS == 'Enabled' ? true : false;
		...
		/**
		 * Returns a string with special text.
		 */
		public function get_special_text_link():String{
			var str:String = '';
			return str;
		}
		...
		public function confirmation(obj:Object):Array{
			var arr:Array = new Array();
			if(obj != null){
				arr.push({label:'Payment Method',	value:Constants.CASHPMT_FORM_LABEL});
		...
		private function set_field_values(type:String):Array{
			...
			if(type == 'meta'){
				arr.push({column:		1,
						 ...
						 form_label:	Constants.CASHPMT_FORM_LABEL.toUpperCase(),
						 tab:			1,
						 tab_enabled:	false,
