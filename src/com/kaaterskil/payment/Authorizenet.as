...
package com.kaaterskil.payment{
	...
	public class Authorizenet extends Sprite{
		...
		public var _special_text_link:String
		 ...
		 * Module description
		 */
		public static const AUTHORIZENET_FORM_DESCRIPTION = 'Please review the following information for accuracy. To make changes, click the \'Back\' button below to return to the billing/payment entry page or click the menus above to return to the main registration pages.';
		...
		private function init():void{
			...
			_module_title		= Constants.AUTHORIZENET_FORM_LABEL;
			_module_description	= Authorizenet.AUTHORIZENET_FORM_DESCRIPTION;;
			_amount				= 0;
			_is_enabled			= Constants.PAYMENT_MODULE_AUTHORIZENET_STATUS == 'Enabled' ? true : false;
			_order_status		= Constants.AUTHORIZENET_ORDER_STATUS;
			_payment_method		= Constants.AUTHORIZENET_FORM_LABEL;
			_special_text_link	= 'Are you uneasy about our online security? Click here.';
			
			var arr:Array = Constants.ORDER_STATUS_LIST.split(',')
		...
		public function confirmation(obj:Object):Array{
			...
			var months:Array = Constants.TEXT_MONTHS.split(',');
			var expire_date:String = months[Number(_cc_expiry_month)] + ', ' + _cc_expiry_year;
			
			var arr:Array = new Array();
			if(obj != null){
				arr.push({label:'Payment Method',	value:_payment_method});
		...
		public function submit(pmt:Object):void{
			...
			var customer_email_test:String = Constants.AUTHORIZENET_EMAIL_CUSTOMER ? 'TRUE' : 'FALSE';
			var merchant_email:String = Constants.AUTHORIZENET_EMAIL_MERCHANT ? Constants.PMT_COMPANY_EMAIL : '';
			...
			//set request data
			_request = new Object();
			_request = {x_login:			Constants.AUTHORIZENET_LOGIN,
						x_tran_key:			Constants.AUTHORIZENET_TXNKEY,
						x_version:			'3.1',
			...
			trace(Common.enumerate_obj(_request));
		...
		private function call_host(vars:Object):void{
			...
			var url:String = Constants.get_host() + Constants.FILENAME_AUHORIZENET_PROXY;
			...
			_loader.process_load(url, vars, 'text', 'POST');
		}
		...
		private function parse_host_reply(arr:Array):void{
			...
			trace(Common.enumerate_obj(arr) + '\n' + Common.enumerate_obj(_response))
			
			//TEST FOR CREDIT CARD APPROVAL
			if(parseInt(_response.response_code) != 1){
				//decline or error
				_message = _response.response_reason_text + ' ' + _response.response_notes;
				trace('payment: order_decline/error: ' + _message);
				dispatchEvent(new Event('payment_process_decline'));
			}else{
		...
		private function set_field_values(type:String):Array{
			...
			if(type == 'meta'){
				arr.push({column:		1,
						 ...
						 form_label:	Constants.AUTHORIZENET_FORM_LABEL.toUpperCase(),
						 tab:			1,
						 tab_enabled:	false,
		...
		private function set_text_formats():void{
			...
			_format_value.font		= 'Tahoma';
			_format_value.size		= 11;
		...
		private function set_send_url():void{
			if(Constants.AUTHORIZENET_MODE == 'Live'){
		...
		/**
		 * Authorize.net text for security modal window
		 */
		public function get_special_text():Object{
			var headline:String = 'Secure Shopping at ' + Constants.COMPANY_NAME;
			var body:String = '<p>You can shop at ' + Constants.COMPANY_NAME + ' online with confidence. We have partnered with <a href="http://www.authorize.net/" target="_blank"><u>Authorize.Net</u></a>, a leading payment gateway since 1996, to offer safe and secure credit card transactions for our customers.<br /></p><p>The Authorize.Net Payment Gateway manages the complex routing of sensitive customer information through the credit card and electronic check processing networks (see an &nbsp;<u><a href="http://www.authorize.net/resources/howitworksdiagram/" target="_blank">online payments diagram</a></u>). The company adheres to strict industry standards for payment processing, including:<br /></p><ul><li>128-bit Secure Sockets Layer (SSL) technology for secure Internet Protocol (IP) transactions.</li><li>Industry leading encryption hardware and software methods and security protocols to protect customer information.</li><li>Compliance with the Payment Card Industry (PCI) Data Security Standard.</li></ul><p><br />For additional information regarding the privacy of your sensitive cardholder data, please read the &nbsp;<u><a href="http://www.authorize.net/company/privacy/" target="_blank">Authorize.Net Privacy Policy</a></u><br /></p><p>' + Constants.COMPANY_NAME + ' is registered with the Authorize.Net Verified Merchant Seal program.</p>';
			
			var obj = {headline_text:	headline.toUpperCase(),
					   body_text:		body, 
					   window_type:		'Alert'
					   };
			return obj;
